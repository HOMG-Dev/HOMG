...
public class UnitManager
{
    ...
    public void Register(string unitType, UnitData unitData)
    {
        ...
        {
            Debug.LogError("UnitManager Register Error:以" + unitType + "为键的UnitData已经存在!");
            return;
        }
    ...
    public void Unregister(string unitType)
    {
        if (!_unitManager.ContainsKey(unitType))
        {
            Debug.LogError("UnitManager Unregister Error:以" + unitType + "为键的unitData并不存在!");
            return;
        }
    ...
    public UnitData GetUnitData(string unitType)
    {
        if (!_unitManager.ContainsKey(unitType))
        {
            Debug.LogError("UnitManager GetUnitData Error:以" + unitType + "为键的unitData并不存在!");
            return null;
        }
